...
export default class Extension extends BaseExtension {
	...
	async mount() {
		db.prepare(
			"CREATE TABLE IF NOT EXISTS httpDevices_schemas (id TEXT PRIMARY KEY, schema TEXT, FOREIGN KEY (id) REFERENCES devices(id) ON UPDATE CASCADE ON DELETE CASCADE)",
		).run();
		db.prepare(
			"CREATE TABLE IF NOT EXISTS httpDevices_states (id TEXT PRIMARY KEY, state TEXT, FOREIGN KEY (id) REFERENCES devices(id) ON UPDATE CASCADE ON DELETE CASCADE)",
		).run();
	...
	async findDevicesToPair() {
		// in case of http devices, this is a static list of preset devices
		return [
			{
	...
	async pairDevice(deviceInfo) {
		db.prepare("INSERT INTO httpDevices_schemas (id, schema) VALUES (?, ?)").run(
			deviceInfo.id,
			JSON.stringify(deviceInfo.schema),
		);
		db.prepare("INSERT INTO httpDevices_states (id, state) VALUES (?, ?)").run(
			deviceInfo.id,
			JSON.stringify({}),
		);
	}
	...
	async createDevice(deviceInfo) {
		deviceInfo.schema = JSON.parse(
			await db.prepare("SELECT schema FROM httpDevices_schemas WHERE id = ?").get(deviceInfo.id),
		);
