...
export class Core extends EventEmitter {
		...
		this.getDevicesList().forEach(device => {
			if (this.extensions[device.type]) {
				if (this.extensions[device.type].canCreateDevice) {
					...
					console.error(
						`Extension ${device.type} can't create devices, the device ${device.name}(${device.id}) will not be loaded`,
					);
				}
			} else {
				console.error(
					`Extension ${device.type} not found, the device ${device.name}(${device.id}) will not be loaded`,
				);
			}
		});
	}
	...
	initDb() {
		...
		db.prepare(
			`CREATE TABLE IF NOT EXISTS db_version (id TEXT PRIMARY KEY, version INTEGER)`,
		).run();
		db.prepare(
			`CREATE TABLE IF NOT EXISTS devices (id TEXT PRIMARY KEY, name TEXT, type TEXT)`,
		).run();
		db.prepare(
			`CREATE TABLE IF NOT EXISTS scripts (id TEXT PRIMARY KEY, name TEXT, code TEXT)`,
		).run();
		db.prepare(
			`CREATE TABLE IF NOT EXISTS devices_scripts (device_id TEXT, event TEXT, script_id TEXT, i INTEGER, PRIMARY KEY (device_id, script_id), FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE, FOREIGN KEY (script_id) REFERENCES scripts(id) ON DELETE CASCADE)`,
		).run();
		db.prepare(
			`CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, name TEXT, script_id TEXT, device_id TEXT, time TEXT, state TEXT, FOREIGN KEY (script_id) REFERENCES scripts(id) ON DELETE CASCADE, FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE)`,
		).run();
	...
	getDevicesToPair() {
		...
		for (let extension in this.extensions) {
			if (
				this.extensions[extension].canCreateDevice &&
				this.extensions[extension].findDevicesToPair
			) {
	...
	async pairDevice(extension, data = {}) {
		if (!this.extensions[extension] || !this.extensions[extension].canCreateDevice)
			throw new Error(`Extension ${extension} can't create devices`);

		data.id = data.id || uuid();
		data.id = data.id.replace(/[^a-zA-Z0-9]/g, ""); //sanitize id
		data.name = data.name || data.id;

		if (db.prepare("SELECT * FROM devices WHERE id = ?").get(data.id))
		...
		db.prepare("INSERT INTO devices (id, name, type) VALUES (?, ?, ?)").run(
			data.id,
			data.name,
			extension,
		);
	...
	getDeviceScripts(deviceId) {
		return db
			.prepare(
				"SELECT * FROM scripts LEFT JOIN devices_scripts ON scripts.id = devices_scripts.script_id WHERE devices_scripts.device_id = ? ORDER BY devices_scripts.i",
			)
			.all(deviceId);
	}
