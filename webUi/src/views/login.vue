	...
	<v-sheet class="bg-blue-grey-darken-4 pa-12 d-flex align-center h-100" rounded>
		<v-card class="mx-auto w-100 px-6 py-8" max-width="344">
			<v-form @submit.prevent="submitLogin">
				<div class="text-h4 d-flex align-center justify-center mb-4"><v-icon>mdi-account</v-icon>Sign In</div>
				...
				<v-text-field v-model="password" :readonly="loading" label="Password"
					placeholder="Enter your password" clearable type="password" />

				<v-alert v-if="error" type="error">{{ error }}</v-alert>
				
				<br>

				<v-btn :loading="loading" color="success" size="large" type="submit"
