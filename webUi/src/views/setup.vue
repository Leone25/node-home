		...
		>
			...
			<template #item.2>
				<v-card title="Create first user" flat>
					...
					/>
					<v-text-field
						v-model="firstUser.confirmPassword"
			...
			<template #item.3>
				<v-card title="Done!" flat
					>You are now ready to start using your Node-Home instance.<br />Head into the settings to
					start adding devices.</v-card
				>
			</template>

			<template #actions="{ next }">
				<div class="d-flex flex-row-reverse px-8 pb-5">
					<v-btn v-if="tab != 3" @click="next" color="primary" :loading="loading" ref="next"
						>Next</v-btn
					>
					<v-btn v-else color="primary" to="/settings" ref="next">Finish</v-btn>
				</div>
			</template>
		</v-stepper>
	</v-sheet>
