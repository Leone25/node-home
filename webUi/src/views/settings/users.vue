...
export default {
	...
	methods: {
		...
		handleCreateUser() {
			this.loading = true;
			this.error = null;
			this.createUser(this.editing).then(() => {
		...
		handleEditUser() {
			this.loading = true;
			this.error = null;
			this.editUser(this.editing.id, this.editing).then(() => {
		...
		handleDeleteUser() {
			this.loading = true;
			this.error = null;
			this.deleteUser(this.editing.id).then(() => {
...
<template>
	...
	<template #item.blocked="{ value: blocked }">
		<v-chip :color="blocked ? 'red' : 'green'" size="small" :text="blocked ? 'Blocked' : 'Active'"/>
	</template>
	...
	<v-dialog max-width="600" v-model="editingDialog" :close-on-back="loading" :persistent="loading">
		<v-card :disabled="loading">
			...
			<v-card-text>
				<v-text-field label="Username" v-model="editing.username" />
				<v-text-field label="Password" hint="Leave empty to not change" :persistent-hint="true" v-model="editing.password" />
				<v-checkbox v-model="editing.blocked" label="Blocked" />
				<v-select
					v-model="editing.roles"
					:items="roles"
			...
			<v-card-actions>
				<v-btn color="error" class="me-auto" @click="deleteDialog = true" :loading="loading">Delete</v-btn>
